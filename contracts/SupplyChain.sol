...
contract SupplyChain {
  ...
  modifier paidEnough(uint _price) { 
     require(msg.value >= _price, "The buyer has not paid the required sum!"); 
    _;
  }
  ...
  // modifier forSale
  // Using require(items[_sku].state == ForSale) will not work because of the enum
  // property that the first element of the enum is indexed by 0. 
  // However the price will not be zero for items on sale.

  modifier forSale (uint _sku) {
    require(items[_sku].price != 0, "Price is zero for items on sale!");
    _;
  }
